...
def depth_plot(stats):
    ...
    stats_ = stats.isel(seed = 0,latitude = 0, domain = 1, temperature = 1, sigma = range(1,4),co2= 0,training_depth = range(1,8))
    

    
    for sigma_i,r2corr in itertools.product(range(3),range(2)):
        sigma = stats_.sigma.values[sigma_i]
        stats = stats_.isel(sigma = sigma_i)
        lsrp = lsrp_.isel(sigma = sigma_i)
